...
:::{grid-item-card} Truncated Y-Axes
Starting a bar chart at a value other than zero can make small differences look dramatic.
:::

:::{grid-item-card} Cherry-Picked Timeframes
Showing only the data that supports your argument while hiding contradictory periods.
:::

:::{grid-item-card} Misleading 3D Effects
3D charts often distort proportions, making some values appear larger than they are.
:::

:::{grid-item-card} Wrong Chart Types
Using a pie chart for data that doesn't represent parts of a whole creates confusion.
:::

::::

```{admonition} Critical Thinking Tip
:class: warning
...
Now that you understand *why* we visualize, let's explore *how*. In {doc}`visualization-types`, you'll learn about the different types of visualizations and when to use each one.
