...
:::{grid-item-card} 🎯 What You'll Learn
...
:::{grid-item-card} 🧠 Big Questions to Consider
...
:::{grid-item}
```{button-link} https://www.tableau.com/
:color: primary
:outline:
Tableau
```
...
:::{grid-item}
```{button-link} https://powerbi.microsoft.com/
:color: primary
:outline:
Power BI
```
...
:::{grid-item}
```{button-link} https://lookerstudio.google.com/
:color: primary
:outline:
Looker Studio
```
...
:::{grid-item}
```{button-link} https://www.datawrapper.de/
:color: primary
:outline:
Datawrapper
```
...
Don't worry—we'll be using **Python** in this course, which is free and incredibly powerful. The {doc}`hands-on-charts` notebook will walk you through it step by step.
...
* - Assignment
  - Due
  - Points
* - Weekly Reflection (Module 7)
  - Friday by class
  - Part of Reflection grade
* - Continue: Usability Project
  - See syllabus
  - Part of project grade
* - Continue: Research Questions
  - See syllabus
  - Part of research grade
```
...
Ready to see the world through a new lens? Start with {doc}`why-visualize` to understand the "why" behind data visualization, then explore the different {doc}`visualization-types` you'll encounter in the wild.

When you're ready to get hands-on, the {doc}`hands-on-charts` notebook will guide you through creating your own visualizations—no prior coding experience required!
