---
banner: images/module-12-banner.png
thumbnail: images/module-12-banner.png
---

# Module 12: Artificial Intelligence 🤖

**Week 12 | The AI Revolution**

---

## The Big Picture

Artificial Intelligence is transforming every aspect of our lives, from how we search for information to how we receive medical diagnoses. AI chatbots, image generators, and AI agents have moved from science fiction to everyday tools. But with great power comes great responsibility.

This module explores AI: what it is, how it works, and what it means for our future.

### 🎯 What You'll Learn

- Understand what AI is and how it has evolved
- Learn the basics of machine learning and neural networks
- Explore current AI applications and their implications
- Understand the emerging legal and regulatory landscape for AI
- Consider the ethical challenges of AI development

### 🧠 Big Questions to Consider

- What can AI do well? What can't it do?
- How do we ensure AI systems are fair and unbiased?
- What jobs will AI change or replace?
- Who is responsible for AI's mistakes?
- Will AI save the world or end it?

---

## This Week's Journey

### 📚 Core Readings & Videos

| Resource | Type | Notes |
|----------|------|-------|
| PBS Nova: A.I. Revolution | Documentary (53 min) | Monday video (link on Canvas) |
| Geoffrey Hinton: Will AI Save the World or End it? | Video | The "Godfather of AI" perspective (link on Canvas) |

### 🔬 Explore & Experiment

- [Hugging Face Spaces](https://huggingface.co/spaces) – Try thousands of AI models yourself, free, in the browser

### 🤔 Make You Think

- AI-generated deepfakes in schools and elections: who should be liable? (Connect to Module 9's TAKE IT DOWN Act)
- Educators worry about AI cheating; students worry about AI-driven grading. Is there a version of AI in education both sides would trust?
- AI and art: if a model trained on a million artists produces a beautiful image, who, if anyone, is the artist? (Connect to Module 10's cases)

---

## Module Contents

This module contains two sections:

1. **[What Is AI and How Does It Work?](what-is-ai.md)** - What AI is, the three types of AI, how machine learning works, and a tour of the generative AI landscape
2. **[AI Ethics, Law, and Policy](ai-ethics-law-policy.md)** - The key ethical concerns, the alignment problem, the arriving laws and regulations, and this course's AI Use Policy

### 🗝️ Key Terms This Week

*Artificial intelligence · Machine learning · Neural network · Large language model · Generative AI · Hallucination · Alignment · AI agent*; see the [Glossary](../glossary.md).

---

:::{seealso} Keep Learning at Dominican 🎓
Ready to go from AI user to AI builder (or AI critic)? These Dominican courses go deeper:

- **CPSC 425 Artificial Intelligence**: the classic algorithms and ideas underneath the headlines.
- **CPSC 428 Applied Machine Learning**: hands-on model building and training.
- **INF 410 Code and Power**: how code frames thought and concentrates power (prerequisites: INF 110 and INF 120).
- **INF 400 Information Ethics and Policy**: the ethics and policy questions AI keeps raising (prerequisites: INF 110 and INF 120).

Talk with your advisor (or Dr. Bowman!) about fitting these into your plan; course details are in the Dominican course bulletin.
:::

---

## 📝 This Week's Assignments

| Assignment | Due | Notes |
|------------|-----|-------|
| Weekly Reflection | Friday by class | Connect to AI concepts |
| Research Presentations | Coming up | Prepare your presentation |
| AI Journal | Due soon | Keep those entries coming |

:::{tip} Reflection Prompt
Reflect on your experience with AI throughout this course and your AI Journal. How has your understanding changed? What excites you about AI? What concerns you? How do you see AI affecting your future career or life?
:::

---

## Let's Get Started!

Start with [What Is AI and How Does It Work?](what-is-ai.md) to get the definitions and the machine learning basics down, then move to [AI Ethics, Law, and Policy](ai-ethics-law-policy.md) for the harder questions: what could go wrong, and who gets to decide.

---

## Looking Ahead

One more frontier: next week we look at **Robotics & Generative AI**, which asks what happens when AI gets a body, and when it gets the keys to act on your behalf.
